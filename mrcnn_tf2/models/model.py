"""
Mask R-CNN
The main Mask R-CNN models implementation.
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    """
    # During models building, Keras calls this function with
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if config.GPU_COUNT > 1:
            from mrcnn_tf2.models.parallel_model import ParallelModel
    ...
    def find_last(self):
        ...
        if not dir_names:
            ...
            raise FileNotFoundError(
                errno.ENOENT,
                "Could not find models directory under {}".format(self.model_dir))
        # Pick last directory
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.log_dir = os.path.join(self.model_dir, f'{datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d")}{self.config.NAME.lower()}')
...
class MeanAveragePrecisionCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch > 2 and (epoch+1)%self.calculate_map_at_every_X_epoch == 0:
            ...
            mAPs, precision, recall = self._calculate_mean_average_precision()
            mAP = np.mean(mAPs)
            ...
            self._verbose_print(f"mAP at epoch {epoch + 1} is: {mAP} Precision: {precision}, Recall: {recall}")

        super().on_epoch_end(epoch, logs)

    def _load_weights_for_model(self):
        last_weights_path = self.train_model.find_last()
        self._verbose_print("Loaded weights for the inference models (last checkpoint of the train models): {0}".format(
            last_weights_path))
    ...
    def _calculate_mean_average_precision(self):
        ...
        for image_id in self.dataset_image_ids[:self.dataset_limit]:
            ...
            AP, precision, recall, _ = utils.compute_ap(gt_bbox, gt_class_id, gt_mask, r["rois"],
                                           r["class_ids"], r["scores"], r['masks'])
            mAPs.append(AP)

        return np.array(mAPs), precision, recall